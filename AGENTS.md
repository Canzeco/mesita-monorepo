...
- **ONE repo (since 2026-07-11): `Canzeco/mesita-monorepo`** — the whole product, one `.git`, one history: `apps/{admin,business,consumer,landing,mobile}` + `supabase/`. The six former standalone repos are frozen read-only history — never work in them. Package rules: each package's `CLAUDE.md`.
- **Alone + small fix?** → branch off fresh main, work, squash-PR, merge it yourself, create the one-line issue at merge time (Ops & maintenance). That's the whole loop.
- **Other agents live on the repo?** → full SWARM: pick → claim (`claimed: <platform>:<session-slug> · branch:<actual-branch>`) → isolated worktree → merge.
...
- **CI is path-filtered per package** (`.github/workflows/{admin,business,consumer,landing,mobile,supabase}.yml` + `rules.yml`) — a PR only runs the pipelines of what it touches.
...
- **Preview servers** (`.claude/launch.json`): admin :3001 · business :3002 · consumer :3003 · landing :3004 · mobile :8081 (`expo start --web`).
