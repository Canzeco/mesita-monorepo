...
**Notion is the library; it wins on any conflict.** This block mirrors [**Rules**](https://www.notion.so/Rules-3d9a9bf37a5280aaab2ed8af665485f7) §0 (`stamp: v4 2026-09-18`; unequal stamps are drift). **Rules = the law** (how change is allowed). **📚 [Docs](https://www.notion.so/Docs-3bfa9bf37a52801e891ec3407d717273) = the knowledge**, one flat page per domain. **Code = this repo.** Boot cards (this block, `AGENTS.md`, package `CLAUDE.md`, skills) mirror §0 and package law into the repo, never knowledge. Linear is the ledger, not a source of truth. **Rules beat Docs.** Two lifecycles: Code → SADLC (monorepo). Docs → KADLC. Changing this Rules tree is not a third lifecycle — issue + Pato for constitution, then SADLC if boot or a gate changed. SADLC / KADLC = Software / Knowledge Agentic Development Life Cycle. Mixed completes every Integrate. Fetch Rules, [SADLC](https://www.notion.so/3d9a9bf37a52800986a7f7f1363e2700), [KADLC](https://www.notion.so/3d9a9bf37a5280f18aa3f73ed6ab8edf) and [Markdowns](https://www.notion.so/3d9a9bf37a52803ebe4cf73bf300f1a3) only; the Map is Rules §4. The retired law pages are in the Trash.
...
**Redeye.** When Pato writes `activate Redeye` — any interface, local or cloud — read `scripts/redeye.md` and obey it until he says stop: no question of any kind, every change committed and its PR merged at once, then the next issue. Mode 6; the picker's five are not it.
...
- **Vercel:** each `apps/web-*` is its own Vercel project (canzeco team) on this repo, Root Directory the package, "skip unaffected" on — a push to `main` deploys only what changed.
