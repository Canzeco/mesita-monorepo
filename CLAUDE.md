...
- **The consumer app is behind a hard phone-OTP wall.** `apps/web-consumer/src/app/(shell)/layout.tsx` calls `getUser()` and redirects anonymous visitors to `/?next=…`, so `/`, `/home`, `/search`, etc. render the sign-in page in a browser even though the server component *does* fetch real places first (the middleware "public browsing" comment is superseded by this layout gate). A logged-in end-to-end browse therefore needs a real phone + Twilio SMS OTP — not available without a test-login secret. The **live backend is reachable with the public key** (e.g. `POST https://<ref>.supabase.co/functions/v1/consumer-web-list-places`), which is the fastest way to smoke-test discovery without auth.
