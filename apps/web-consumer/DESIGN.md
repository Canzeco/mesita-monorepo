...
| **Rewards wallet** | `shrink-0` header — a boxed `PitchSteps` module over the tab track — above **one column** of ticket rows | The four steps are the wallet's **masthead — always visible, on every tab, never gated** (Pato, 2026-08-10; supersedes MESITA-1018's New-only scoping), boxed so the header reads as two stacked controls rather than loose marks. Header never scrolls: an orientation control that scrolls away has stopped orienting. Tickets are scanned for state and QR at arm's length — never gridded |
| **THE TICKET** (`/rewards/ticket/[id]`) | Real route, rendered as an **80%-tall bottom-anchored panel** | Rounded top + card fill so the 20% of page ground above reads as a sheet edge, not as content that failed to reach the top. Still a route — the `@modal` interceptor is dead and stays dead (MESITA-857). Scroll lives inside the panel so the shape never grows |
