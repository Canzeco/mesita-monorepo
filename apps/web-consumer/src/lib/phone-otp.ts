// Phone-OTP helpers: E.164 assembly, per-country length validation, and
// human-readable copy for the errors Supabase Auth + Twilio actually return.
//
// Why this exists: the sign-in form used to send whatever digits the user
// typed straight to Supabase, which forwards them to Twilio. A short or
// trunk-prefixed number comes back as a raw provider string —
// "Error sending confirmation OTP to provider: The 'To' number +52555...
// is not a valid phone number. More information: https://twilio.com/..."
// — which is both unreadable and a wasted Twilio lookup we pay for.
//
// Mirrored by hand in apps/mobile-consumer/src/lib/phone-otp.ts. Keep the
// two in step (same convention as consumer-route-contract.ts).
...
/**
 * Assemble strict E.164 (`+<dial><national>`, no spaces) from the picker's
 * country plus whatever the user typed. Supabase Auth rejects anything
 * looser, and Twilio bills us for the lookup either way.
 */
...
/**
 * Map a Supabase Auth error to copy a guest can act on. Falls through to
 * the provider's own message so we never swallow something unexpected.
 */
