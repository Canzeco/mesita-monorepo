...
export function CatalogRails() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setFetchError(err instanceof Error ? err.message : "Couldn't load the catalog");
        setRails([]);
      }
    })();
    return () => {
  ...
  if (rails === null) {
    return (
      <div className="flex min-h-0 flex-1 flex-col gap-8 overflow-y-auto px-4 pt-4 pb-6">
        {Array.from({ length: 3 }, (_, i) => (
          <div key={i} className="space-y-3">
            <Skeleton className="h-4 w-28" />
            <div className="flex gap-2.5 overflow-hidden">
              {Array.from({ length: 4 }, (_, j) => (
                <Skeleton key={j} className="h-52 w-[148px] shrink-0 rounded-2xl" />
              ))}
            </div>
          </div>
        ))}
  ...
  return (
    <div className="scrollbar-hide min-h-0 flex-1 overflow-y-auto px-4 pt-4 pb-6">
        ...
        {rails.map((rail) => (
          <section key={rail.key} aria-labelledby={`${rail.key}-heading`}>
            ...
            <ul
              ...
              className="scrollbar-hide mt-3 -mx-4 flex gap-2.5 overflow-x-auto px-4 pb-1"
            >
