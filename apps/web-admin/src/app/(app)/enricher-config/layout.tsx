...
// the Enricher is the cron pipeline that builds place profiles from the open
...
export default function EnricherConfigLayout({
  ...
  return (
    <ConfigPageLayout
      ...
      title="Intake"
      description="Intake is how a place gets into Mesita and becomes a profile. The Enricher, its engine, is the cron pipeline that builds that profile out of the open web: one CREATE run at birth — seed, pulse, details — then a queue of nine functions, re-run per place on its own decay schedule. This page was its console: which events start a run, which subprocesses each run may buy, the caps and models it buys them with, and what one run costs. Those settings are live and every Edge Function still reads them; what is gone is the console for changing them, which had grown into a wall of knobs no operator could read straight through. The page stays empty until it is worth reading again. What each setting means lives in Notion Docs › Enrichment."
    >
      {children}
    </ConfigPageLayout>
  );
