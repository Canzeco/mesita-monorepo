...
- Light theme + semantic tokens; calm and high-density — don't ornament. Design map: [`DESIGN.md`](./DESIGN.md). Shared chrome: [`src/components/admin-ui/`](./src/components/admin-ui/) (`config` · `manage` · `lineup`). New admin UI imports from there; do not grow the route-local shims.
