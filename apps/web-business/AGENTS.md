...
- **Nav:** LEFT RAIL, seven flat pages, no boxes: Account · Organization (org + place switchers beneath it; its list and Add step light it) · Place Profile · Reviews · Activity · Settings · Admin. `AppShell.tsx` (`fixed inset-0` + `overflow-clip`) resolves scope once (`lib/rail-scope.ts`); `main` is the only scroller; one breakpoint `lg`. `Sidebar.tsx`: `--sidebar`, `w-60`/`w-16`, zero indents, one ink pill. Every page has `loading.tsx`. `ConsoleHeader.tsx`: crumb + route.
