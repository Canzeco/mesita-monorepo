// Promos Playground — the reward-distribution model (MESITA-991, v12 in
// MESITA-1705).
//
// Given the v12 config plus operator assumptions (visit mix, class mix,
...
//
// THE PLAN MIX IS GONE (MESITA-1705). A "what share pays for Premium" number
// used to split every class two ways, because the plan moved the rate. It no
// longer does, so the assumption would change no output — a knob that moves
// nothing is worse than no knob.
//
// This is the ONLY distribution model left: the admin console had a twin and
// its whole Expected Distribution box was removed in the same change.
//
...
export function distributionFor(
  ...
  for (const cls of CLASS_KEYS) {
    const pp = clampPct(assumptions.classPct[cls]) / classSum;
    if (pp === 0) continue;
