...
export type RowState =
  /** Not in the catalogue at all: create it, then it is theirs. */
  ...
  /** Already this organization's: the only thing left to do is open it. */
  | { kind: "open"; label: "In this organization"; placeId: string }
  /** This organization asked and is waiting. */
  | { kind: "pending"; label: "Your claim is pending"; placeId: string }
  /** Another organization HOLDS it. NO BUTTON — see below. */
  | { kind: "taken"; label: "Claimed" };

/** The lookup, this organization's holdings, and nothing else.
 *
 *  A HELD PLACE GETS NO BUTTON (Pato's call, MESITA-1850). Every action the
 *  screen could offer here would 409 against a place another organization
 *  holds, and an honest dead end beats a button that lies. The cost is real
 *  and named: an operator who IS the true owner of a place someone else
 *  grabbed has no path on this screen and has to reach ops. A contest flow is
 *  the upgrade when that first actually happens.
 */
export function rowStateForLookup(
  ...
  // Held by THIS organization wins over every other state: whatever the
  // catalogue thinks, the operator's own place opens.
  if (heldPlaceIds.has(lookup.place.id)) {
    return { kind: "open", label: "In this organization", placeId: lookup.place.id };
  }
  ...
  // places)". Nobody HOLDS it; another organization merely asked. Whoever
  // actually completes the claim gets it, and hiding the button here would
  // let a pending request no one ever finishes park a place forever.
  if (
  ...
  // verified_partner: an organization HOLDS it.
