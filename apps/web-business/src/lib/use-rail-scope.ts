...
// header, so the three can never disagree about which place is on screen —
// the drift MESITA-1713 found when each resolved it alone. The rule itself is
// pure (lib/rail-scope.ts); this hook only feeds it the pathname and the
// session's last place.
import { usePathname } from "next/navigation";
import { useLastPlaceId } from "@/components/console/OpenPlace";
import { resolveRailScope, type RailPlace, type RailScope } from "@/lib/rail-scope";

export function useRailScope(input: {
  places: readonly RailPlace[];
  rememberedPlaceId: string | null;
  /** The places read failed (`(shell)/layout.tsx`). Carried, never
