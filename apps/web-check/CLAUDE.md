...
- **URL flip still pending:** live QRs continue to encode `mesita.ai/check/<code>` (served by web-landing) until the EF `CHECK_URL_BASE` flips — the project and domain now exist, so MESITA-814 is down to that one switch. While the duplication window is open, mirror any check-flow change into `apps/web-landing` in the same PR (the flip plan recommends serving codes at the domain root: `check.mesita.ai/<code>`).
