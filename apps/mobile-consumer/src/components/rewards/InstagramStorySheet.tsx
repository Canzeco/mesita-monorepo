...
import type { ProofSheetPhase } from '@/components/rewards/GoogleReviewSheet';
...
function InstagramStorySheetBody({
  onConfirm,
  onClose,
}: {
  ...
  return (
    <View style={{ padding: 16, paddingBottom: 32, gap: 12 }}>
      ...
      >
        Tag the place in your story, then confirm here so we can unlock the
        reward.
      </Text>
...
export function InstagramStorySheet({
  open,
  onClose,
  placeName,
  onConfirm,
  ...
  return (
    ...
    >
      {open ? (
        <InstagramStorySheetBody onConfirm={onConfirm} onClose={onClose} />
      ) : null}
    </FullScreenSheet>
  );
}
