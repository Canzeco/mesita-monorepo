// Overview enrichment for place cards — REAL data only (RN port of
...
// timezone, zone, city, address, enriched_at. They just aren't on the
// `Place` *type*, so nothing mapped them onto the derived overview names
// (google_rating, open_now, last_updated_label, …) that the card reads.
//
...
//
// PURE + presentational: no state, no fetch, no dependency on
// use-home-deck — each render site (swipe card, catalog card, map card)
// applies it to the row it already holds. The `??` chains are real-vs-real
// (an already-mapped value wins over a freshly-derived one), never
// real-vs-fabricated: a genuinely missing cell stays null and its chip
// simply hides.
//
...
export function enrichPlaceOverview(v: Place): Place {
  ...
  // Neighborhood (zone) priority: the real zone column → the colonia parsed
  // out of the formatted address → the city. Most places have no zone column
  // yet but DO carry the colonia inside `address`, so deriving it here lights
  // up the neighborhood chip without a DB backfill, and degrades to city when
  // the address has none (e.g. US places).
  const zone =
  ...
  return {
    ...
    // tooltip. The per-tier rate itself is resolved off the raw row columns
    // and only for Verified Partners (web-listed places never offer rewards).
...
// adapters/place-to-detail-helpers.ts). Kept local so this module doesn't
// widen that file's export surface just to read two scalars.
