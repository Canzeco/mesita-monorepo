...
//
// MIRROR OF `apps/web-consumer/src/lib/passport-document.ts`. Same logic, same
// literals, same comments — only the quote style differs. Change one, change
// the other in the SAME PR: the two platforms must never print a different
// document for the same guest, because consumer IA cannot diverge. There is no
// test runner on mobile (package.json has only lint and typecheck), so the
// contract that pins this logic lives on web at
// `src/lib/__tests__/passport-document.test.ts` and covers both by mirroring.
//
// This module is PURE: no React, no React Native, no Supabase, no NativeWind.
// `src/app/(tabs)/me/passport.tsx` renders off it.
//
// NOTHING NEW IS FETCHED. Every value below is already on the profile the
// AuthProvider holds (code, first_name, last_name, sex, birthday, phone).
// No EF, no migration, no new column.
//
...
// hidden from the screen reader on both platforms: 44 characters of `<` read
// aloud is hostile, and the field grid above already announces every fact it
// encodes.
...
export type PassportDocumentInput = {
   ...
   *  (see COUNTRIES.iso3 in lib/countries.ts). Passed in rather than derived
   *  here so this module stays free of the country table. */
 ...
 *  'Complete' link to Me › Profile beside the count. Web's
 *  `passport-axes.test.ts` pins the passport's href list to exactly two doors,
 *  which is MESITA-1801's law and not this issue's to spend — and mobile has
 *  to match web anyway. The count alone still tells the guest what is missing;
