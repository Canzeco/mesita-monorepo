...
- **Web look + feature parity (2026-07-11 — supersedes “similar-not-identical”).** Mobile must look **very alike** to `apps/web-consumer` (same IA, screens, tokens, brand moments) and ship **exactly the same features/functionalities**. Use **react-native-paper** + Expo/RN SDKs to build faster — never hand-roll design-system primitives from `Pressable`/`View`/`Text`. Libraries facilitate; they do not excuse missing features or a divergent look. Sole deliberate exception: **no Stripe/subscribe checkout UI** in the iOS binary (Apple review) — Premium/Class UI still match web; subscribe happens on web. Keep Home/Search/Me/Rewards/Reservations/place detail aligned with the web surface (including parked-vs-live: if web parks Memo/Social, mobile parks them too; if web has a built Rewards page behind a parked tab, mobile must have that page).
