...
- **Known parity gap — the wallet.** This screen still renders the OLD `PayClient` (QR passport + ticket list) against web's bare place list opening THE TICKET. Port web's shape when you next touch this tab; the current screen is not the target.
