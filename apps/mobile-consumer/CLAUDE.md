...
- **UI kit, not hand-rolled primitives (2026-07-11).** Use a prebuilt React Native component library for Button / Input / Sheet / Modal / Card / Tabs / etc. Do **not** rebuild a design system from `Pressable`/`View`/`Text` + NativeWind. Default kit: **react-native-paper** (MD3, Mesita-branded theme in `src/constants/paper-theme.ts`). gluestack-ui v5 needs NativeWind v5 — deferred while this package stays on NativeWind 4. Product-specific composites (SwipeDeck, Ask AI, MesitaMark, map) stay custom; everything else comes from the kit. **Web parity is similar-not-identical** — this is a prototype; do not pixel-chase `apps/web-consumer`.
