...
// A SNAPSHOT of apps/web-business/src/components/place-manage/PlaceSaveBar.tsx.
//
// The page's ONE save. It replaces the per-card SaveBars that used to sit
// inside Basics, Hours, Channels and Photos — four footers asking the same
// question, each answerable only by finding the amber dot inside the card you
// happened to edit. Here the answer is one line, in one place, and it names
...
//
// One difference from the real file, and it is a mount point rather than a
// line of markup: there it is rendered by the place LAYOUT, so every tab gets
// it without any tab knowing it exists. Here Profile is the only editable
// screen, so it renders it itself. `main` is still the scroller either way,
// which is all `sticky bottom-0` needs.
          ...
          {savePending ? (
            ...
            <>
              ...
              <span className="min-w-0">
                <span className="text-sm font-semibold">{count} unsaved</span>
                {count > 0 ? (
