-- MESITA-1055, part 2 — finish the manual_priority removal in the two
-- profiles INSTEAD OF trigger bodies.
--
-- WHY A SECOND MIGRATION. 20260823071643 (part 1) dropped the column and
-- rebuilt both trigger bodies from the then-current pg_get_functiondef. Between
-- that migration and this one, a concurrent session applied
-- 20260823071739_drop_reservation_endpoint_contacts, which rebuilt the same two
-- functions from an older source. plpgsql bodies are cumulative and whole-body:
-- that rebuild carried `manual_priority` back in while dropping the
-- reservation_endpoint/reservation_contacts writes. Because part 1 had already
-- dropped places.manual_priority, the reinstated bodies referenced a column
-- that no longer exists, and Postgres does not validate plpgsql at CREATE time
-- — so the breakage only surfaces at runtime, as
--   ERROR 42703: column "manual_priority" of relation "places" does not exist
-- on EVERY insert and update through public.profiles.
--
-- This migration re-strips manual_priority from the CURRENT installed bodies.
-- It deliberately preserves 20260823071739's removal of the
-- reservation_endpoint / reservation_contacts writes rather than reverting a
-- concurrent session's work: those columns no longer exist either.
--
-- Only the two functions change here. The view, its
-- with (security_invoker = true) reloption, its grants, its comment and both
-- triggers were verified untouched and are left alone.
