-- MESITA-926 — Place About: English core (`description`) + Spanish translation
-- (`description_es`). Mesita's canonical language is English; Spanish rides as
-- a translation layer. Enricher always writes both. Consumer About shows both.

alter table public.places
  add column if not exists description_es text;

comment on column public.places.description is
  'Canonical About — English (Mesita core language). Enricher always writes English.';
comment on column public.places.description_es is
  'Spanish translation of About. Enricher generates from English core; admin/business editable.';

-- Recreate projects_view with description_es APPENDED (CREATE OR REPLACE maps
-- by position). MUST keep security_invoker (MESITA-599 invariant).
...
-- INSTEAD OF triggers enumerate place columns — route description_es too.
