-- MESITA-1208 — order + reservation routing gets real columns.
--
-- Routing (which contact an order or a reservation reaches the place on) was
-- stored inside places.products, the jsonb whose declared contract is the MENU
-- ("Generic product payload… Example: {"menu":[...]}", 20260531234000). That
-- forced every writer to read-modify-write the whole blob: the Enricher needed
-- mergeProductsReservations() purely to avoid clobbering the menu, and both
-- admin cards rewrote the blob from client-held state, so a stale tab could
-- silently drop the menu (or a menu save drop the routing). Four typed columns
-- end that: independent writes, a real CHECK, no merge helper.
--
-- Columns land on places because that is where routing was modelled from the
-- start — reservation_endpoint / reservation_contacts (20260710025836) were
-- added for exactly this job, then abandoned in favour of the blob.
--
-- 'phone' is the ONLY permitted channel: it is the only serving path
-- (MESITA-842 — the Reservationist is voice-only; MESITA-839 removed WhatsApp
-- fleet-wide). A stored 'whatsapp' would be silently undialled, which is the
-- exact bug MESITA-842 filed. Widening the CHECK ships WITH the serving code
-- that needs it — an unenforced config is a bug.
--
-- The legacy pair stays for now (0 rows, already inert): removing it means
-- DROP VIEW + rebuilding grants and triggers, so it belongs in the CONTRACT
-- follow-up rather than here. This migration is append-only on the view.

alter table public.places
  add column if not exists reservation_channel text,
  add column if not exists reservation_target  text,
  add column if not exists order_channel       text,
  add column if not exists order_target        text;

comment on column public.places.reservation_channel is
  'Serving channel the Reservationist dials. phone only (MESITA-842). Null = unset.';
...
comment on column public.places.order_channel is
  'Serving channel an order reaches the place on. phone only. STAGED: stored now, read when the order rail ships (MESITA-1155).';
comment on column public.places.order_target is
  'Contact value for order_channel.';

-- CHECK constraints, idempotent (Postgres has no ADD CONSTRAINT IF NOT EXISTS).
do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid = 'public.places'::regclass
      and conname = 'places_reservation_channel_check'
  ) then
    alter table public.places
      add constraint places_reservation_channel_check
      check (reservation_channel is null or reservation_channel in ('phone'));
  end if;

  if not exists (
    select 1 from pg_constraint
    where conrelid = 'public.places'::regclass
      and conname = 'places_order_channel_check'
  ) then
    alter table public.places
      add constraint places_order_channel_check
      check (order_channel is null or order_channel in ('phone'));
  end if;
end
$$;

-- Backfill out of the blob. Only 'phone' survives: a legacy whatsapp/instagram
-- pick was never dialled (MESITA-842), so it is not an override worth carrying
-- and the Enricher re-seeds phone on the next run.
update public.places p set
  reservation_channel = case
    when p.products->'reservations'->>'channel' = 'phone' then 'phone' end,
  reservation_target  = case
    when p.products->'reservations'->>'channel' = 'phone'
      then nullif(btrim(coalesce(p.products->'reservations'->>'value', '')), '') end,
  order_channel = case
    when p.products->'orders'->>'channel' = 'phone' then 'phone' end,
  order_target  = case
    when p.products->'orders'->>'channel' = 'phone'
      then nullif(btrim(coalesce(p.products->'orders'->>'value', '')), '') end
where p.products ?| array['reservations', 'orders'];

-- profiles carries the new columns so the admin's single write path
-- (business-web-update-project → profiles → INSTEAD OF trigger) reaches them
-- with no second endpoint. CREATE OR REPLACE appends at the end, which keeps
-- the existing grants (anon/authenticated SELECT, service_role write) and both
-- INSTEAD OF triggers bound — no DROP VIEW.
--
...
-- Both INSTEAD OF bodies are rebuilt from the LIVE pg_get_functiondef (Dev
-- Rules §B: PL/pgSQL bodies are cumulative and are never rebuilt from a repo
-- copy — the repo's last copy predates the projects_view→profiles rename and
-- the mesita_name/manual_priority writes). Only the four routing columns are
-- added; every other line is the live body verbatim.
