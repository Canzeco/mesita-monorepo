...
-- rebuilt public.profiles without it. Both INSTEAD OF bodies were left naming it
-- — profiles_insert in its column list AND its VALUES list, profiles_update in
-- its SET list — so every write through public.profiles raised "column
-- manual_priority of relation places does not exist". The place create and
-- update paths were down.
--
