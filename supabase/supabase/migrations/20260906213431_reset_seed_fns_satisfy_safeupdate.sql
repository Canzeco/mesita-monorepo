-- The `authenticator` role (which PostgREST — and so the admin-web-reset-database
-- EF's `admin.rpc()` call — connects through) preloads the `safeupdate` extension:
-- it blocks any UPDATE/DELETE lacking a WHERE clause for the whole session,
-- including inside a SECURITY DEFINER function called via RPC. A direct
-- superuser SQL connection doesn't hit it, which is why static review of these
-- function bodies looked clean while the live button aborted with
-- `reset_failed: UPDATE requires a WHERE clause` (MESITA-1589).
--
-- admin_reset_database() calls seed_place_super_categories() first, whose
-- first statement was a bare UPDATE — the reset aborted there, before the
-- wipe or anything else ran. seed_place_categories() had the same shape.
-- `where true` is functionally identical (still touches every row) and
-- satisfies the guard.
...
as $function$
begin
  update public.place_super_categories set sort_order = sort_order + 1000 where true;
...
as $function$
begin
  update public.place_categories set super_category_slugs = '{}'::text[] where true;
