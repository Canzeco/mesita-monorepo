-- Undefined Super Category (7th guest pill) + Undefined Category membership.
-- Partition stays exclusive: the Atlas slug `undefined` maps to Super
-- `undefined` only. Google `other` (hotels, shops, schools) stays ineligible
-- and is not this Super. No Google search battery — selecting only Undefined
-- does not Nearby-fill hotels. Create path stamps family_keys = {undefined}.

insert into public.place_super_categories (slug, label, emoji, sort_order)
values ('undefined', 'Undefined', '❓', 7)
...
comment on function public.seed_place_super_categories() is
  'Admin/reset seed for place_super_categories (7 guest pills, 5–10 band). Service-role only.';
...
begin
  ...
  update public.place_categories set super_category_slugs = array['undefined']
...
comment on column public.places.family_keys is
  'Inferred Super Category (0–1). Create path stamps {undefined} until contents infers a classified Category.';

insert into public.place_categories (slug, label, section, sort_order)
...
update public.place_categories
   set super_category_slugs = array['undefined']::text[]
 where slug = 'undefined';

update public.places
   set family_keys = array['undefined']::text[]
