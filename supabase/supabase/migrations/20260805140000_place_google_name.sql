-- MESITA-917 — Dual place names: google_name (Enricher spine) + name (Mesita display).
-- Approach B from CEO plan 2026-08-05-place-google-vs-mesita-name.
--
-- Backfill caveat: google_name = name until the next full re-enrich repairs the
-- Google spine for any places that were already admin-renamed.
...
where google_name is null and name is not null;

comment on column public.places.google_name is
  'Google Places displayName — Enricher spine; not admin-editable.';
comment on column public.places.name is
  'Mesita display name. Empty/null ⇒ UI falls back to google_name.';

-- Recreate projects_view with google_name APPENDED (CREATE OR REPLACE maps by
-- position). MUST keep security_invoker (MESITA-599 invariant).
...
-- INSTEAD OF triggers enumerate place columns — route google_name too.
...
begin
  insert into public.places (
    ...
    coalesce(new.manual_priority, 0.1), new.google_name
