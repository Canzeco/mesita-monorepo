...
//
// v12 (MESITA-1705): the payload is the CONTEXT × CLASS Promos config —
// visits (base: strategy × class, + bonuses), orders (base: strategy alone,
// + bonuses, parked) and the default cap. The PLAN axis is gone: a reward is
// base + welcome + class + actions, and nothing the guest pays for moves it.
// It is written as the `v12` key on app_config.promos_config, MERGE-preserving
// the blob's other keys. A WHOLE-BLOB write: the caller always sends the
// complete config and normalizePromos always returns a complete one.
//
...
Deno.serve(async (req) => {
  ...
  if (shape === "stale-v10" || shape === "stale-v11") {
    console.warn(
      `rewards-config: refused a stale ${shape.slice(6)} save from user ${userId}`,
    );
    return jsonError(
