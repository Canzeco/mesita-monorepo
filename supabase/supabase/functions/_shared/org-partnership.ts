...
//
// This file no longer writes anything on `organizations`. It holds the three
// patch builders and the one writer that move a single PLACE between
// `plan=free` and `plan=pro Zero`, and nothing else.
//
// WHO CALLS IT. `_shared/partner-membership.ts` — the Mesita Membership's
// entitlement writer (MESITA-1877) — is the only path that cascades a whole
// organization, and `business-web-claim-place` joins one newly claimed place
// when its holder organization is already partnered.
//
// WHO WRITES THE ORG BIT. `organizations.partnered` is written by the
// Membership lifecycle ALONE: business-web-start-membership under
// MOCK_SUBSCRIPTION, and stripe-webhook-handle-event once Stripe confirms.
// `setOrgPartnership` used to live here and was the operator's second door
// onto the same fact — it also wrote `organizations.mesita_pay_enabled` in
// the same statement and refused ON without a Ready Connect account.
// MESITA-1889 retired it with its EF (the source is archived under
// retired/edge-functions/business-web-set-org-partnership). Mesita Pay is its
// own switch with its own writer; buying a partnership never switches card
