...
import { crenupPromptsMeta } from "./crenup-prompts.ts";
...
Deno.test("every prompt-bearing Crenup step is on the payload, in pipeline order", () => {
  const keys = crenupPromptsMeta().map((p) => p.key);
  assertEquals(keys, [
    "scout",
    "resolver",
    "category",
...
Deno.test("the payload ships the SAME instructions the pipeline sends", () => {
  // The drift guard. `crenup-prompts.ts` imports these constants rather than
  ...
  const byKey = new Map(crenupPromptsMeta().map((p) => [p.key, p]));
...
Deno.test("'Agent X' and 'Agent Y' are gone from every rendered string", () => {
  ...
  for (const p of crenupPromptsMeta()) {
...
Deno.test("the Resolver template renders every optional branch", () => {
  ...
  const resolver = crenupPromptsMeta().find((p) => p.key === "resolver")!;
