...
const fullQueue = () => CRENUP_LADDER.map((p, i) => done(p, i));

Deno.test("crenup: every stamped step matches the DB's step CHECK", () => {
  ...
  for (const piece of CRENUP_LADDER) {
    assertEquals(
      DB_CHECK.test(`S${CRENUP_STEP_META[piece].index}`),
      true,
      `step S${CRENUP_STEP_META[piece].index} (${piece}) violates the DB CHECK`,
    );
  }
  // Embedding is function 8, so reportCrenupSteps stamps it S8. S10 stays
...
Deno.test("crenup: the EIGHT stamped functions, in the decided order", () => {
  ...
  assertEquals(CRENUP_TOTAL, 8);
});

Deno.test("crenup: `seed` is function 0 and is NEVER stamped", () => {
  ...
  assertEquals((CRENUP_LADDER as readonly string[]).includes("seed"), false);
  assertEquals(CRENUP_LADDER[0], "details");
  assertEquals(CRENUP_STEP_META.details.index, 1);
  ...
  assertEquals(crenupHighWater(events), 8);

  // And a stray seed beacon cannot inflate a place that has done nothing.
  assertEquals(crenupHighWater([done("seed", 1)]), 0);
});
...
Deno.test("crenup: PULSE and MENU are not steps — they are retired keys", () => {
  ...
  // never ran.
  assertEquals((CRENUP_LADDER as readonly string[]).includes("pulse"), false);
  assertEquals((CRENUP_LADDER as readonly string[]).includes("menu"), false);
  assertEquals(CRENUP_RENAMES.pulse, undefined);
  assertEquals(CRENUP_RENAMES.menu, undefined);
  for (const key of ["pulse", "menu"]) {
    assertEquals((CRENUP_RETIRED as readonly string[]).includes(key), true);
  }

  // A legacy place whose ONLY stamp is `pulse` reads 0, not 1.
  assertEquals(crenupHighWater([done("pulse", 1)]), 0);
  // And a `menu` row sitting where slot 7 used to be neither advances the
  // queue nor blocks it.
  assertEquals(
    crenupHighWater([done("details", 1), done("menu", 2)]),
    1,
  );
});

Deno.test("crenup: CREATE's stamp reads 1/8 — one ladder, two callers", () => {
  ...
  assertEquals(crenupHighWater(created), 1);
  const b = crenupBlockedAt(created);
  assertEquals(b?.key, "serp");
  assertEquals(b?.index, 2);
  assertEquals(b?.state, "missing");
});

Deno.test("crenup: an UN-QUEUED create jumps to 8 with 2–6 still a gap", () => {
  ...
  assertEquals(crenupHighWater(doorCreate), 1);
  assertEquals(completedCrenupSteps(doorCreate), [
    "details",
    "description",
    "embedding",
  ]);
...
Deno.test("crenup: `embedding` is ONE function now, not two extras", () => {
  assertEquals((CRENUP_LADDER as readonly string[]).includes("name"), false);
  assertEquals((CRENUP_LADDER as readonly string[]).includes("summary"), false);
  assertEquals((CRENUP_LADDER as readonly string[]).includes("semantic"), false);
  assertEquals((CRENUP_LADDER as readonly string[]).includes("embedding"), true);
  assertEquals(CRENUP_STEP_META.embedding.index, 8);
  assertEquals(CRENUP_STEP_META.embedding.label, "Embedding");
  assertEquals([...CRENUP_EXTRAS], []);
});

Deno.test("crenup: the RENAMED `semantic` still counts as function 8", () => {
...
Deno.test("crenup: rows from the PREVIOUS ladder still read correctly", () => {
  ...
  assertEquals(crenupHighWater(legacy), 3);
...
Deno.test("crenup: social runs BEFORE images — the gathers fill the pools", () => {
  ...
  const i = (k: string) => (CRENUP_LADDER as readonly string[]).indexOf(k);
  assertEquals(i("social") < i("images"), true);
  assertEquals(i("links") < i("social"), true);
  assertEquals(i("images") < i("description"), true);
});

Deno.test("crenup: the two Apify gathers sit together at 4 and 5", () => {
  ...
  // be asked.
  assertEquals(CRENUP_STEP_META.social.index, 4);
  assertEquals(CRENUP_STEP_META.reviews.index, 5);
});

Deno.test("crenup: serp runs BEFORE links — that is what serp is FOR", () => {
...
Deno.test("high water: Embedding is 8 — a gap before it still reads 7", () => {
  const throughDescription = CRENUP_LADDER
    .filter((p) => p !== "embedding")
    .map((p, i) => done(p, i));
  assertEquals(crenupHighWater(throughDescription), 7);
  assertEquals(crenupHighWater(fullQueue()), CRENUP_TOTAL);
  assertEquals(
    crenupHighWater([
      ...throughDescription,
  ...
  assertEquals(crenupHighWater([done("embedding", 1)]), 0);
});
...
Deno.test("high water: nothing recorded is 0 — SEEDED, not a failure", () => {
  assertEquals(crenupHighWater([]), 0);
});

Deno.test("high water: a full queue is 8", () => {
  assertEquals(crenupHighWater(fullQueue()), 8);
});
...
Deno.test("high water: it is HOW FAR, not how many", () => {
  ...
  const events = CRENUP_LADDER
    .filter((p) => p !== "links")
    .map((p, i) => done(p, i));
  assertEquals(crenupHighWater(events), 2);
  // completedCrenupSteps answers the OTHER question — which ones landed.
  const landed = completedCrenupSteps(events);
...
Deno.test("high water: a function a run did not buy simply has no event", () => {
  ...
  assertEquals(crenupHighWater([...earlier, ...refresh]), 4);
});
...
Deno.test("high water: never exceeds the total, and never goes negative", () => {
  const n = crenupHighWater([...fullQueue(), ...fullQueue()]);
  assertEquals(n >= 0 && n <= CRENUP_TOTAL, true);
  assertEquals(n, 8);
});
...
// reportCrenupSteps ever writes one, so the next person to add a beacon cannot
...
Deno.test("no raw beacon may use a CRENUP step key as its step_name", async () => {
  ...
  const pieces = new Set<string>(CRENUP_LADDER);
  ...
  assertEquals(
    offenders,
    [],
    `A raw beacon is using a function key as step_name, which corrupts the high-water. Use reportCrenupSteps, or rename the beacon.`,
  );
});
...
Deno.test("blocked: a permanently-closed place is FAILED at 1, not merely absent", () => {
  ...
  assertEquals(crenupHighWater(events), 0);
  const b = crenupBlockedAt(events);
  assertEquals(b?.key, "details");
  ...
  assertEquals(crenupHighWater(legacy), 0);
  assertEquals(crenupBlockedAt(legacy)?.state, "missing");
});
...
Deno.test("blocked: it never disagrees with the high-water", () => {
  ...
  for (const events of cases) {
    const hw = crenupHighWater(events);
    const b = crenupBlockedAt(events);
    if (hw === CRENUP_TOTAL) {
...
Deno.test("blocked: seed is never the blocker — it is not a function", () => {
  ...
  for (const events of [[], [done("pulse", 1)], fullQueue()]) {
    // The type itself now forbids "seed" (CrenupStep excludes it) — this
    // compares as strings so the test survives even a type regression.
    assertEquals((crenupBlockedAt(events)?.key as string | undefined) === "seed", false);
  }
});
...
Deno.test("crenupOf: the shared reader off places.enrichment (MESITA-1598)", () => {
  assertEquals(crenupOf({ highWater: 7 }), 7);
  assertEquals(crenupOf({ highWater: 0 }), 0);
  // Malformed or absent reads the CREATED floor, never throws.
  assertEquals(crenupOf(null), 0);
  assertEquals(crenupOf(undefined), 0);
  assertEquals(crenupOf("not an object"), 0);
  assertEquals(crenupOf({}), 0);
  assertEquals(crenupOf({ highWater: "nope" }), 0);
  assertEquals(crenupOf({ highWater: -3 }), 0);
  // Clamped at CRENUP_TOTAL and truncated — never a fractional or out-of-range
  // rung, whatever a stray write puts in the jsonb.
  assertEquals(crenupOf({ highWater: CRENUP_TOTAL + 5 }), CRENUP_TOTAL);
  assertEquals(crenupOf({ highWater: 3.9 }), 3);
});
