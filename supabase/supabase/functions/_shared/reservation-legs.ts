...
//
//   consumer → business  ("business_booking")     the agent calls the place on
//                        behalf of the guest to REQUEST the reservation.
//   business → consumer  ("guest_confirmation")   after the place confirms,
...
export type ReservationLegVars = {
  placeName: string;
  guestName: string;
  /** The guest-side callback number the place may be left with. */
  guestPhone: string;
  /** The ticket's 8-digit reference code — speakable if either side asks. */
  referenceCode: string;
  partySize: number;
  /** Place-local es-MX date, e.g. "sábado 2 de agosto". */
  dateEs: string;
  /** Place-local es-MX time, e.g. "8:30 p.m.". */
  timeEs: string;
   ...
   * asks the place to MOVE the reservation instead of booking a stranger's. */
...
/** Leg 1 · consumer → business: request the reservation from the place. */
export function businessLegPrompt(v: ReservationLegVars): string {
  ...
  const modification = v.modificationOf
    ...
    : "";
  return [
    `Eres el asistente de reservaciones de Mesita. Esta llamada va del consumidor hacia el negocio: llamas al restaurante ${v.placeName} DE PARTE del comensal ${v.guestName}.`,
...
export function guestLegPrompt(v: ReservationLegVars): string {
  return [
    `Eres el asistente de reservaciones de Mesita. Esta llamada va del negocio hacia el consumidor: llamas al comensal ${v.guestName} DE PARTE del restaurante ${v.placeName}.`,
    `El restaurante YA CONFIRMÓ la reservación. Tu único objetivo es avisarle y confirmarle al humano los datos: ${v.partySize} ${v.partySize === 1 ? "persona" : "personas"}, el ${v.dateEs} a las ${v.timeEs} en ${v.placeName}. Si pregunta por una referencia, su código es ${v.referenceCode}.`,
...
// ── Cancellation notices (Reservations Rules §B legs 5 & 6) ───────────────
...
export function placeCancelNoticePrompt(v: ReservationLegVars): string {
  return [
    `Eres el asistente de reservaciones de Mesita. Esta llamada es un AVISO al restaurante ${v.placeName}: el comensal ${v.guestName} CANCELA su reservación de ${v.partySize} ${
...
export function placeCancelNoticeFirstMessage(v: ReservationLegVars): string {
  return `¡Hola, buenas! Le llamo de Mesita por la reservación a nombre de ${v.guestName}.`;
}

/** Leg 6 · business → consumer: the place cancelled — the guest must know. */
export function guestCancelNoticePrompt(v: ReservationLegVars): string {
  return [
    `Eres el asistente de reservaciones de Mesita. Esta llamada es un AVISO al comensal ${v.guestName}: el restaurante ${v.placeName} tuvo que CANCELAR su reservación del ${v.dateEs} a las ${v.timeEs}.`,
...
export type GuestLegContext = {
  ...
  callContext?: string;
  /** Speakable list of the place's alternatives — "" when none. */
  placeAlternatives?: string;
};
