...
// `_shared/place-taxonomy.ts` (Atlas membership = the one Super Category
// when the category is in the catalog; stored keys only when
// membership is empty; else the Google primaryType map). Clients must
// not re-mirror that expansion; Search map Filters cut on Super
// Category off this field.
//
