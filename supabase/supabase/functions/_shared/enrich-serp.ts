// Atlas SERP summary — Step S3, the Scout (Perplexity Agent, pro-search). A fast
...
// The Resolver's link selection (enrich-channel-discovery.ts) AND the final synthesis.
// Shares the one Perplexity Agent client (perplexity-agent.ts) with the Resolver.
...
// The Scout — SERP AI Summary (child G / MESITA-204). This blurb is SOFT grounding
...
export function buildScoutInput(place: {
  ...
  const { name, locationLine, category } = place;
  return `Research the place "${name}"` +
...
export async function gatherSerpSummary(opts: {
  ...
  const prompt = buildScoutInput({ name, locationLine, category });
