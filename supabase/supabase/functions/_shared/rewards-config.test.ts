...
// Pure, no DB. resolveTicketRate maps (posture, grid, context) -> the single
...
Deno.test("resolveTicketRate: class rates on a returning visit (aggressive)", () => {
  assertEquals(resolveTicketRate("aggressive", GRID, { classKey: "free", isFirstVisit: false }), 10);
...
Deno.test("resolveTicketRate: Welcome joins the set on a first visit", () => {
  assertEquals(resolveTicketRate("aggressive", GRID, { classKey: "free", isFirstVisit: true }), 30);
...
Deno.test("resolveTicketRate: verified actions bump, best-of never stacks", () => {
  ...
  assertEquals(
    resolveTicketRate("aggressive", GRID, { classKey: "free", isFirstVisit: true, reviewVerified: true }),
    50,
  );
...
Deno.test("resolveTicketRate: zero posture pays nothing", () => {
  assertEquals(
...
Deno.test("placePosture: derives from v4 columns, dominant → aggressive", () => {
  // Conservative preset (v4 tens grid).
  assertEquals(
    placePosture({ welcome_free_rate: 20, welcome_premium_rate: 30, free_rate: 10, premium_rate: 20 }),
    "conservative",
  );
  // Retired Dominant preset coerces to the nearest surviving posture.
  assertEquals(
    placePosture({ welcome_free_rate: 40, welcome_premium_rate: 50, free_rate: 20, premium_rate: 30 }),
    "aggressive",
  );
  // Custom / all-null → zero.
  assertEquals(
    placePosture({ welcome_free_rate: null, welcome_premium_rate: null, free_rate: null, premium_rate: null }),
    "zero",
  );
});
