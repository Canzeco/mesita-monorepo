...
// inherits and the bonus that goes with it. Exists so the Wallet stops
// hard-coding a number the console claims to own — an admin knob nothing reads
// is the "unenforced config = bug" failure root CLAUDE.md names.
//
// Returns the GUEST slice only. maxHoldHours and minHoldHours are operator
// policy about what a PLACE may choose; a guest reads the hold on their own
// card, not the range a venue was allowed to pick from.
//
