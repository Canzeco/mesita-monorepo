...
Deno.serve(async (req) => {
  ...
  const existing = await admin
    .from("consumers")
    .select("id, code, full_name, first_name, last_name, sex, birthday, country, phone, instagram_handle, profile_public, profile_show_saves, profile_show_visits, profile_show_stories, class_key, class_origin, consumer_instagram_followers_count, class_expires_at")
    .eq("id", userId)
    .maybeSingle();
  if (existing.error) {
