...
Deno.test("a ticket-rail intent (ticket_id metadata, no mesita_kind) does not route to Credits", () => {
  const event = paymentIntentEvent({ ticket_id: "ticket_1", consumer_id: "c_1" });
  assertEquals(isCreditPurchaseIntentEvent(event), false);
});

// ─── The backstop handler ───────────────────────────────────────────────

function fakeAdmin(): { admin: SupabaseClient; rpcCalls: { fn: string; args: unknown }[] } {
  const rpcCalls: { fn: string; args: unknown }[] = [];
  const admin = {
    rpc: (fn: string, args: unknown) => {
      rpcCalls.push({ fn, args });
      return Promise.resolve({ data: { ok: true, lotId: "lot_1" }, error: null });
    },
