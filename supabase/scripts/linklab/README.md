...
Finds the best way to discover a place's **official website** and **official Instagram** from just
...
- `supabase/functions/_shared/linklab/` — the reusable engine (imported by BOTH the runner and the EF)
  ...
  - `context.ts` — per-place context: Google resolve + seed website + one Perplexity SERP summary
...
- `scripts/linklab/ground_truth.json` — verified 50-place truth set
...
returning `null` for a place that genuinely has no website counts as correct (TN). A confident-but-
...
MESITA-192). Note the ground-truth set is small (n≈50) and famous-place-optimistic — directional only.
